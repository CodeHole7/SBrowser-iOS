...
- (CookieJar *)init
{
	...
	{
		/* migrate old whitelist entries to new HostSettingsSBrowser entries */
		...
		if ([fileManager fileExistsAtPath:whitelist]) {
			...
			NSLog(@"[CookieJar] migrating old cookie whitelist to HostSettingsSBrowser: %@", list);
			for (NSString *host in [list allKeys]) {
				HostSettingsSBrowser *hc = [HostSettingsSBrowser for:host];
	...
	self.oldDataSweepTimeout = [NSNumber numberWithDouble:SettingsSBrowser.cookieAutoSweepInterval];
...
{
	return [HostSettingsSBrowser for:host.lowercaseString].whitelistCookies;
}
...
#ifdef TRACE_COOKIES
		NSLog(@"[CookieJar] [TabSBrowser h%lu] storing %lu cookie(s) for %@ (via %@)", tabHash, [newCookies count], [URL host], mainDocumentURL);
#endif
...
#ifdef TRACE_COOKIES
	NSLog(@"[CookieJar] [TabSBrowser h%lu] touched data access for %@", tabHash, domain);
#endif
}
...
- (void)clearNonWhitelistedDataForTab:(NSUInteger)tabHash
{
...
#ifdef TRACE_COOKIES
	NSLog(@"[TabSBrowser h%@] clearing non-whitelisted data", tabHashN);
#endif
	...
	for (NSString *cookieDomain in [[[self dataAccesses] objectForKey:tabHashN] allKeys]) {
		...
		if (blocker) {
#ifdef TRACE_COOKIES
			NSLog(@"[TabSBrowser h%@] data for %@ in use on tab %@, not deleting", tabHashN, cookieDomain, blocker);
#endif
		}
		else if (![self isHostWhitelisted:cookieDomain]) {

#ifdef TRACE_COOKIES
			NSLog(@"[TabSBrowser h%@] deleting data for %@", tabHashN, cookieDomain);
#endif
